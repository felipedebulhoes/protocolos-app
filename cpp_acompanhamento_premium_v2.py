#!/usr/bin/env python3
"""
Script para redesenhar o bloco de Acompanhamento Premium em protocolos cirúrgicos.
Estrutura: Pré-op extenso (vitaminas), Durante, Pós-op 6 meses com acesso 24/7.
"""
...
SURGICAL_PROTOCOLS = {
    "1_implante_protese_peniana": {
        ...
        "type": "cirurgia",
        "pre_op_focus": "Otimização cardiovascular e psicológica",
        "post_op_duration": "6 meses",
    },
    "1b_protese_semirrigida": {
        "procedure": "Implante de Prótese Peniana Semirrígida",
        "type": "cirurgia",
        "pre_op_focus": "Otimização cardiovascular",
        "post_op_duration": "6 meses",
    },
    "2_varicocelectomia": {
        "procedure": "Varicocelectomia",
        "type": "cirurgia",
        "pre_op_focus": "Otimização de fertilidade e função testicular",
        "post_op_duration": "6 meses",
    },
    "5_micro_tese": {
        "procedure": "MicroTESE (Extração de Esperma)",
        "type": "procedimento",
        "pre_op_focus": "Otimização de fertilidade",
    ...
    "13_hpb_manejo_completo": {
        ...
        "type": "cirurgia",
        "pre_op_focus": "Otimização renal e cardiovascular",
        "post_op_duration": "6 meses",
    },
}
...
def create_acompanhamento_premium_v2(protocol_name, details):
    ...
    pre_op_focus = details["pre_op_focus"]
...
def apply_new_acompanhamento(protocols_json_path, dry_run=True):
    """Aplica o novo bloco de Acompanhamento Premium aos protocolos."""
    ...
    for protocol_id, details in SURGICAL_PROTOCOLS.items():
        ...
        changes.append({
            ...
            "procedure": details["procedure"],
            "sections_before": len(sections) - 1,
    ...
    if not dry_run:
        ...
        print(f"✅ {len(changes)} protocolos atualizados")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("REDESENHO: Acompanhamento Premium v2 com Acesso 24/7")
    print("=" * 80)
    print()
    
    changes, _ = apply_new_acompanhamento(protocols_path, dry_run=dry_run)
    
    print(f"Protocolos a atualizar: {len(changes)}")
    for change in changes:
        print(f"  ✓ {change['procedure']} ({change['protocol']})")
        print(f"    Seções: {change['sections_before']} → {change['sections_after']}")
