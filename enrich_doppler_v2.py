...
def enrich_doppler_v2():
    ...
    # Encontrar o protocolo de Disfunção Erétil
    de_protocol = None
    for p in protocols:
        if p["id"] == "4_disfuncao_eretil":
            de_protocol = p
            break
            
    if de_protocol:
        # Inicializar chaves se não existirem
        if "atestados" not in de_protocol:
            de_protocol["atestados"] = []
        if "laudos" not in de_protocol:
            de_protocol["laudos"] = []
            
        # Adicionar os laudos estruturados de Doppler Peniano
        de_protocol["atestados"].append({
        ...
        print("Laudos de Doppler Peniano integrados no protocolo de Disfunção Erétil com sucesso!")
    else:
        print("Erro: Protocolo de Disfunção Erétil não encontrado no JSON!")
